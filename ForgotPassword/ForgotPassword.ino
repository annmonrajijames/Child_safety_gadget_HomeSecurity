...
unsigned long previousMillis = 0; // Stores the last time "HELLO WORLD" was displayed
const long interval = 1000; // Interval at which to display "HELLO WORLD" (milliseconds)
...
void loop() {
  handleKeypad(); // Other tasks can be performed here without being interrupted by password handling
  HelloWorld(); 
}
...
void handleKeypad() {
  ...
  if (key != NO_KEY) {
    passwordHandling(key); // Call the password handling function only when a key is pressed
  }
}

void passwordHandling(char key) {
...
void HelloWorld() {
  ...
  if (currentMillis - previousMillis >= interval) {
    // Save the last time you displayed "HELLO WORLD"
    previousMillis = currentMillis;
    
    // Your regular "HELLO WORLD" message here
